...
- (void)didLoadFromCCB {
    ...
    //note that the bush ratio is longer than the cloud
    _bushParallazRatio = ccp(0.9, 1);
    _cloudParallaxRatio = ccp(0.5, 1);
    
    for (CCNode *bush in _bushes) {
        CGPoint offset = bush.position;
        [self removeChild:bush];
        [_parallaxBackground addChild:bush z:0 parallaxRatio:_bushParallazRatio positionOffset:offset];
    }
...
{
    _parallaxBackground.position = ccp(_parallaxBackground.position.x - (character.physicsBody.velocity.x), _parallaxBackground.position.y);
